...
- **Global scope**: `C:/dev/script-dev/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/dev/script-dev/.opencode/notes/`)
...
2. Glob `C:/dev/script-dev/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (C:/dev/script-dev/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/dev/script-dev/.opencode/notes/` — works in any directory
